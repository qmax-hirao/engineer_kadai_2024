...
public class FruitManager : MonoBehaviour
{
    ...
    public void MoveGeneratePos()
    {
        ...
        // 左右キーで移動する
        if (Input.GetKey(KeyConfig.instance.leftMoveKey))
        {
            pos = new Vector2(putPos.position.x - moveSpeed * Time.deltaTime, putPos.position.y);
        }
        else if (Input.GetKey(KeyConfig.instance.rightMoveKey))
        {
            pos = new Vector2(putPos.position.x + moveSpeed * Time.deltaTime, putPos.position.y);
        }
    ...
    {
        if (Input.GetKeyDown(KeyConfig.instance.fruitPutKey) && !isCoolTime)
        {
            // SE
